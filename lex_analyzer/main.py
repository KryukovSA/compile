...
from lex_analyzer.generate_code import else_labels, end_labels, generate_assembly_code
...
def p_program_heading(p):
    ...
    if len(p) == 3:
        p[0] = Node(
            "PROGRAM_HEADING", [Node("PROGRAM"), Node("IDENTIFIER", value=p[2])]
        )
    else:
        p[0] = Node(
            "PROGRAM_HEADING", [Node("PROGRAM"), Node("IDENTIFIER", value=p[2]), p[4]]
        )
...
def generate_assembly_code_final(node):
    if node.type == "PROGRAM":
        ...
        code = ""
        code += f"global {node.children[1].value.value}\n"
        return code
        # return ""
        ...
        constant_value = node.children[1].value.value.value
        for child in node.children:  # ---------------------------
            code += generate_assembly_code_final(child)  # --------------------------
    ...
    # типо IDENTIFIER его дублирует
        ...
        for child in node.children:
            code += generate_assembly_code_final(child)
        number_value = node.value.value.value
    ...
    #         code += generate_assembly_code_final(child)
        ...
        asm_code += (
            f"    mov eax, {type_identifier} ; Move type identifier into EAX register\n"
        )
        return asm_code
    ...
    # вызывает дублирование
        ...
        condition_code = generate_assembly_code_final(
            node.children[0]
        )  # Генерация кода для условия
        loop_code = generate_assembly_code_final(
            node.children[1]
        )  # Генерация кода для тела цикла
        ...
        if operator == "*":
            ...
            asm_code += (
                " add esp, 4  ; Adjust the stack pointer to remove one operand\n"
            )
        ...
        asm_code = ""
        generate_assembly_code_final(node.children[0])  # Вычисление условия
        asm_code += "cmp eax, 0"
        else_label = f"else_{len(else_labels)}"
        end_label = f"end_{len(end_labels)}"
        asm_code += f"je {else_label}"  # Переход к блоку else при равенстве нулю
        generate_assembly_code_final(node.children[1])  # Генерация кода блока then
        asm_code += f"jmp {end_label}"  # Переход в конец условного оператора
        asm_code += f"{else_label}:"
        if len(node.children) == 3:
            generate_assembly_code_final(node.children[2])  # Генерация кода блока else
        asm_code += f"{end_label}:"
        return asm_code
    else:
        return ""
